...
def test_walk_sequence_number(monkeypatch):
    ...
    runner = CliRunner()
    result = runner.invoke(
        kines_cli.kines, ["walk", "test-stream-walk", "000000000102", "-s", "49612939735146152444019686740765606284716713833062727682"], input="n\n"
    )
    print("walk output", result.output)
...
def test_walk_latest(monkeypatch):
    ...
    runner = CliRunner()
    result = runner.invoke(
        kines_cli.kines, ["walk", "test-stream-walk", "000000000102", "-l"], input="n\n"
    )
    print("walk output", result.output)
