...
class MockKinesisClient:
    ...
    def get_records(*args, **kwargs):
        return tests.test_boto3_client_responses.KINESIS_RECORDS_ITERATOR_1_RESPONSE
...
def test_walk(monkeypatch):
    monkeypatch.setattr(boto3, 'client', mock_get_kinesis_client)
    runner = CliRunner()
    result = runner.invoke(kines_cli.kines, ['walk', 'test-stream-walk', '000000000102'], input='n\n')
